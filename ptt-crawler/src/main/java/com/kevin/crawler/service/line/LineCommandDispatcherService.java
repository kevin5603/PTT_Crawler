...
public class LineCommandDispatcherService {
  ...
  public void messageDispatcher(LineInfoDto dto) {
    ...
    Matcher versionMatch = Pattern.compile(LineConst.VERSION_PATTERN).matcher(message);

    try {
      if (addKeywordMatch.matches()) {
        String board = addKeywordMatch.group(1).toLowerCase().trim();
        String keyword = addKeywordMatch.group(2).trim();
        ...
        String board = removeKeywordMatch.group(1).toLowerCase().trim();
        String keyword = removeKeywordMatch.group(2).trim();
        log.info("userId: {} Remove the keyword '{}' from the board '{}'", userId, keyword, board);
        keywordService.removeKeyword(userId, board, keyword);
        String response = String.format("Remove the keyword: '%s' from the '%s' board", keyword,
          board);
        sendReplyMessage(dto, response);
        ...
        // TODO sent git log to user
        sendReplyMessage(dto, "TODO sent git log to user");
      } else {
