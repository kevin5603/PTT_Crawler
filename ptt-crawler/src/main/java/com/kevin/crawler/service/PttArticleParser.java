...
public class PttArticleParser {
  ...
  public List<Article> parseArticles(String html) {
    ...
    Document doc = Jsoup.parse(html);
    String board = doc.select("a.board").getFirst().ownText().toLowerCase();
    Elements elements = doc.select("div.r-ent");
    for (Element element : elements) {
      // The data will be automatically deleted after three hours
      Long expire = (System.currentTimeMillis() / 1000) + (3 * 60 * 60);
      ...
      list.add(new Article(link, author, title, board, expire));
    }
    return list;
  }
